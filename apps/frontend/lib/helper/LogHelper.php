...
EOD;
    for($x = 0; $x < count($weapons); ++$x) {
      $w = $weapons[$x];
      $s .= '<th colspan="2" class="ui-state-default">'.outputWeapon($w['name'], $w['key_name'], $w['image_name'])."</th>";
    }
    $s .= "</tr><tr>";
    for($x = 0; $x < count($weapons); ++$x) {
...
EOD;
      for($x = 0; $x < count($miniStats); ++$x) {
        $st = $miniStats[$x];
        $s .= '<th><img src="/avatars/'.$st['playerId'].'.jpg" class="'.$st['team'].' avatarImage ui-corner-all" title="'.$st['name'].'"/></th>';
      }
    $s .= '</tr></thead><tbody>';
  for($x = 0; $x < count($miniStats); ++$x) {
    $stat = $miniStats[$x];
