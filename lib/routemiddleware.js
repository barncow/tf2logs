...
function handleMustBeLoggedIn(req, res) {
  req.flash('error', 'You must be logged in to perform that action.');
  res.redirect('/players/login/interstitial');
}
...
rm.andRestrictToLoggedInUser = function(req, res, next) {
  ...
  else handleMustBeLoggedIn(req, res);
}

/**
  Similar to andRestrictToLoggedInUser, only this does not require a MongoDB call. Handy for uploads, where a call to MongoDB would interrupt.
  Use: app.get('/myroute/, rm.mustBeLoggedIn, function(req, res){doStuff();});
*/
rm.mustBeLoggedIn = function(req, res, next) {
