...
class sfPHPOpenID {
  ...
  public function getRedirectURL($immediate = false, $submitLabel = '')
  {
    ...
    // Choose between GET and POST request method.
    // http://openid.net/specs/openid-authentication-2_0.html#indirect_comm
    // Both are valid. We prefer GET as it allows direct redirection without user interaction.
    // POST can be prefered for big requests (>2kb) not supported by internet explorer.
    // PHP OpenID provide a method ($auth_request->shouldSendRedirect()) to help in decision but it not very pertinent.
    $redirect_url = $auth_request->redirectURL($this->getTrustRoot(),
                                               $this->getApprovedURL(),
                                               $immediate);
    if (strlen($redirect_url) < 2097152) {
  ...
  */
  public function getAuthResult()
  {
    ...
    else if ($response->status == Auth_OpenID_SUCCESS)
    {
        ...
        if ($ax_resp) {
          foreach ($this->request_fields_AX as $alias => $url) {
            $get_ax = $ax_resp->get($url); // $get_ax is an array or an Auth_OpenID_AX_Error object in case of error
            if (is_array($get_ax) && (count($get_ax) > 0))
